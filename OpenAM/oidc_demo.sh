...
# Legacy Endpoint
#TOK=`curl -s -k --request POST --data "username=${USERNAME}&password=${PASSWD}&uri=realm=/{$REALM}" "${AMURL}/identity/authenticatei" | cut -f2 -d=`
...
TER=`curl -s -k --request POST --data "grant_type=authorization_code&code=${AC}&redirect_uri=${REDURI}&client_id=${O2CI}&client_secret=${O2CS}&claims=phone_number" ${ATE}`
...
echo ""
echo "=> User Info"
curl -s -k --request POST --data "access_token=$AT" ${UIE}
echo ""
